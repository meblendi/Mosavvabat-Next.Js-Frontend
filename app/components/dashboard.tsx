...
const Dashboard: React.FC = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useApi<Order[]>("/mosavvabat/orders/");

  if (loading)
    return <div className="flex justify-center">... Dashboard is Loading </div>;
  if (error) return <div>Error: {error}</div>;

  // Get organization logo URL
  const organizationLogo =
    user?.profile?.organization_logo ||
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
          <div>
            ...
            {user?.profile?.position || "تعیین نشده"}
          </div>
          <div>
          ...
          {user?.profile?.can_approve && (
            <div>
              <span className="font-medium">سقف تأیید:</span>{" "}
